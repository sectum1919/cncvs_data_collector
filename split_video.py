...
def save_shot_boundary_img(
    ...
    boundaries = [round(b[0] * 25) for b in boundaries]
    filename = meta['save_file']
    idx = 0
    for b in boundaries:
        idx += 1
        video.set(cv2.CAP_PROP_POS_FRAMES, b - 1)
        ...
        if success:
            cv2.imwrite(os.path.join(dst_dir, f'{filename}_shot_{idx+1}_begin.jpg'), begin_frame)
